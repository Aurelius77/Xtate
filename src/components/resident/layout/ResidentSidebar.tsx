...
import { LayoutDashboard, DollarSign, Calendar, MessageSquare, FileText, Key, QrCode, Menu } from 'lucide-react';
...
const ResidentSidebar = ({ currentPage, setCurrentPage, onLogout }: ResidentSidebarProps) => {
  ...
  const menuItems = [
    ...
    { icon: QrCode, label: 'My Access Codes', page: 'my-access-codes' }
  ];
      ...
      <aside className={`fixed lg:relative inset-y-0 left-0 z-40 w-64 transform ${sidebarOpen ? 'translate-x-0' : '-translate-x-full'} lg:translate-x-0 transition-transform duration-300 ease-in-out flex flex-col gap-6 sidebar-glass p-6`}>
        ...
        <div className="mt-auto glass-card p-4">
          <p className="text-sm leading-snug text-cyan-100">
            Manage your profile and preferences to enhance your resident experience.
          </p>
